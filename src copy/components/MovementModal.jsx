...
export default function MovementModal({ initialProdId = null, onClose }) {
  ...
  const [form, setForm] = useState({ prodId: initialProdId || products[0]?.id || "", type: "entrada", qty: 1, date: today, note: "" });
  ...
  function handleSave() {
    ...
    const result = addMovement({ prodId: Number(form.prodId), type: form.type, qty: Number(form.qty), date: form.date, note: form.note });
    ...
    <Modal title="Movimentar estoque" onClose={onClose}>
      <div className="space-y-4">
        <div>
          ...
          <select value={form.prodId} onChange={(e) => setForm({ ...form, prodId: e.target.value })} className="input">
        ...
        <div>
          ...
          <input type="text" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Opcional" className="input" />
        </div>
