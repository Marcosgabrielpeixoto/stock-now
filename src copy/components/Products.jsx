...
export default function Products() {
  ...
  const filtered = products.filter((p) => {
    ...
    return (!s || p.name.toLowerCase().includes(s) || p.category.toLowerCase().includes(s)) && (!filterCat || p.category === filterCat);
  });
  ...
  function handleSave() {
    ...
    if (modal === "edit") updateProduct(editingId, data); else addProduct(data);
    setModal(null);
  }
  function handleDelete(id) { if (window.confirm("Excluir este produto?")) deleteProduct(id); }

  const statusMap = { ok: "bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400", low: "bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-400", out: "bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-400" };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <input type="text" placeholder="Buscar produto ou categoria..." value={search} onChange={(e) => setSearch(e.target.value)}
          className="border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 w-56 placeholder-gray-400 dark:placeholder-gray-500" />
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-10 text-gray-400 dark:text-gray-500">Nenhum produto encontrado</td></tr>
            ) : filtered.map((p) => (
      ...
      {modal && (
        <Modal title={modal === "edit" ? "Editar produto" : "Novo produto"} onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div><label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Nome</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Categoria</label>
                <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="input">
                  {CATEGORIES.map((c) => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div><label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Unidade</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Quantidade atual</label>
                <input type="number" min={0} value={form.qty} onChange={(e) => setForm({ ...form, qty: e.target.value })} className="input" />
              </div>
              <div><label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Estoque mínimo</label>
